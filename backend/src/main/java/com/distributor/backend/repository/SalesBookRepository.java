...
public interface SalesBookRepository extends JpaRepository<SalesBook,Long> {
    @Query(value = "SELECT * FROM sales_book sb " + "JOIN buyer b ON sb.customer = b.gst " + "WHERE STR_TO_DATE(CONCAT(sb.order_year, '-', LPAD(sb.order_month, 2, '0'), '-', LPAD(sb.order_day, 2, '0')), '%Y-%m-%d') " + "BETWEEN :startDate AND :endDate " + "AND b.gst LIKE CONCAT(:gstPrefix, '%')", nativeQuery = true)
