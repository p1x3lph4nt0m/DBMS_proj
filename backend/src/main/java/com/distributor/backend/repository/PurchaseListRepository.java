...
public interface PurchaseListRepository extends JpaRepository<PurchaseList, PurchaseListId> {
    @Query(value = "SELECT name, weight, company, max_retail_price, selling_price, quantity, discount FROM purchase_list as p, items as t WHERE p.item_id = t.id AND p.bill_number = ?1", nativeQuery = true)
